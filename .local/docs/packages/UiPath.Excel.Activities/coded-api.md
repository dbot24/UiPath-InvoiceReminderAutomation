...
> **Note:** These namespaces are auto-imported **only inside `CodedWorkflow`-derived classes**. In plain `.cs` files (helper classes, test files, or any class that does not extend `CodedWorkflow`), **all four namespaces above must be added explicitly** — this applies equally to portable (`IWorkHandle`) and Windows/Interop (`IWorkbookQuickHandle`) usage:
> ```csharp
> using UiPath.Excel;                        // core types: ExcelValue, WorksheetQuickHandle, ConflictBehavior, etc.
> using UiPath.Excel.Activities;             // activity-level types
> using UiPath.Excel.Activities.API;         // IWorkHandle extension methods, ExcelOperations static methods
> using UiPath.Excel.Activities.API.Models;  // WorkbookOptions, UseOptions, etc. (ConflictBehavior is in UiPath.Excel)
> ```
> Without `using UiPath.Excel.Activities.API;`, calls such as `workbook.ReadRange(...)`, `workbook.GetSheets(...)`, and `handle.Filter(...)` fail with `CS1061`.
>
...
>
> **Name clash across Office packages — `ConflictBehavior`.** The Excel, Word, and PowerPoint coded APIs each declare their **own** `ConflictBehavior` enum — `UiPath.Excel.ConflictBehavior`, `UiPath.Word.Activities.ConflictBehavior`, and `UiPath.Presentations.Activities.ConflictBehavior` (the Excel and PowerPoint packages also each declare a `UseOptions` class; Word's equivalent is named `WordUseOptions` and does not clash). If a single coded workflow references more than one of these packages, an unqualified `ConflictBehavior` becomes **ambiguous** and the compiler reports `CS0104: 'ConflictBehavior' is an ambiguous reference`. Resolve it by preferring the type from the **same package** as the options object you are configuring, written fully-qualified. For example, to replace an already-existing file via `WorkbookOptions.ConflictBehavior`, use `UiPath.Excel.ConflictBehavior.Replace` (or add an alias such as `using ExcelConflict = UiPath.Excel.ConflictBehavior;`). The samples below use the fully-qualified form for this reason.
...
| `InsertImage(string sheetName, CreateImageConfig config)` | `void` | Inserts an image into the specified sheet, anchored at a cell. Configure path, anchor cell, pixel offsets, and optional size via `CreateImageConfig`. Changes saved immediately. |

> For CSV file read/write operations, see [CSV File Operations](#csv-file-operations) — those are standalone static methods that do not require a workbook handle.

> **Alternative static-call form:** All extension methods above are declared on the `ExcelOperations` static class and can also be called in their explicit static form — e.g., `ExcelOperations.ReadRange(workbook, "Sheet1", "A1:D100", true, false)`. Prefer the extension method syntax (`workbook.ReadRange(...)`) in all normal usage; the static form is a fallback for contexts where extension method resolution fails (e.g., certain reflection-based tooling or F# interop).
...
These are standalone static methods on the `ExcelOperations` class — no workbook handle or `UseWorkBook` call is needed. They work cross-platform and operate directly on CSV files on disk.

> **Note:** In plain `.cs` files (outside `CodedWorkflow`-derived classes), add the following `using` directives as needed:
> - `using UiPath.Excel.Activities.API;` — for `ExcelOperations` and the `IWorkHandle` extension methods
> - `using UiPath.Excel.Activities.API.Models;` — for options and model types (`WorkbookOptions`, `UseOptions`, etc.)
> - `using UiPath.Excel.Activities;` — for activity-level types if referenced directly
> - `using UiPath.Excel;` — for core Excel types (e.g. `ExcelValue`, `WorksheetQuickHandle`) and the `ConflictBehavior` enum
> - `using UiPath.CSV;` — for `DelimitatorOptions` (required by the CSV methods)
...
Configuration for inserting an image (portable API).

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `ImagePath` | `string` | `null` | Local file path to the image to insert. |
| `Cell` | `string` | `null` | Anchor cell for the top-left corner of the image (e.g. `"A1"`). |
| `Left` | `int` | `0` | Horizontal pixel offset from the anchor cell. |
| `Top` | `int` | `0` | Vertical pixel offset from the anchor cell. |
| `Width` | `int?` | `null` | Image width in pixels. `null` or `0` keeps the original width (or scales from `Height` if that is set). |
| `Height` | `int?` | `null` | Image height in pixels. `null` or `0` keeps the original height (or scales from `Width` if that is set). |
...
### Use the portable API in a plain C# class (non-CodedWorkflow)

Outside `CodedWorkflow`-derived classes the `UiPath.Excel.Activities.API` namespace is **not** auto-imported. Add all four `using` directives explicitly so the `IWorkHandle` extension methods, `ExcelOperations` static methods, and model types resolve.

```csharp
// Required in any plain .cs file (helper class, test, etc.) — none of these are auto-imported outside CodedWorkflow
using System.Data;                         // DataTable — not covered by .NET implicit usings
using UiPath.Excel;                        // core types: ExcelValue, WorksheetQuickHandle, ConflictBehavior, etc.
using UiPath.Excel.Activities;             // activity-level types
using UiPath.Excel.Activities.API;         // IWorkHandle extension methods, ExcelOperations static methods
using UiPath.Excel.Activities.API.Models;  // WorkbookOptions, UseOptions, etc. (ConflictBehavior is in UiPath.Excel)
...
public class ReportHelper
{
    ...
    public ReportHelper(IExcelService excel) => _excel = excel;

    public DataTable LoadSummary(string path)
    {
        using var workbook = _excel.UseWorkBook(path, createNew: false);
        // Extension methods on IWorkHandle resolve because of the usings above
        return workbook.ReadRange("Summary", "", addHeaders: true, preserveFormat: false);
    }
}
```
...
public void Execute()
{
    var options = new WorkbookOptions
    {
        Path = "C:\\Output\\new_report.xlsx",
        CreateNew = true,
        ConflictBehavior = UiPath.Excel.ConflictBehavior.Replace
    };
...
```csharp
// In a plain .cs file, add these explicitly (auto-imported only inside CodedWorkflow):
// using UiPath.Excel;
// using UiPath.Excel.Activities;
// using UiPath.Excel.Activities.API;
// using UiPath.Excel.Activities.API.Models;
// using UiPath.CSV; // DelimitatorOptions — not auto-imported even inside CodedWorkflow
[Workflow]
public void Execute()
{
    ...
    // Export to CSV using ExcelOperations static method (no handle required; part of UiPath.Excel.Activities.API)
    ExcelOperations.WriteCsvFile("C:\\Output\\filtered.csv", filtered, DelimitatorOptions.Comma,
