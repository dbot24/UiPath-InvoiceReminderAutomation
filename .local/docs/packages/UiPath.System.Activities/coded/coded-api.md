...
> **Note:** These namespaces are auto-imported **only inside `CodedWorkflow`-derived classes**. In plain `.cs` files (helper classes, test files, or any class that does not extend `CodedWorkflow`), add the following `using` directives explicitly:
> ```csharp
> using UiPath.Core;                              // core UiPath types
> using UiPath.Core.Activities.API;              // ISystemService interface itself
> using UiPath.Core.Activities.Storage;          // IStorageFile, IStorageBucketFile
> using UiPath.Orchestrator.Client.Models;       // OrchestratorJob, QueueItem, QueueItemData, etc.
> using UiPath.Activities.System.Jobs.Coded;     // RunJobOptionalParameters, JobFilterSettings, LogEntryType, InvokeProcessTargetSession, etc.
> ```
> Note that `UiPath.Core.Activities.API` (the namespace of `ISystemService`) is **not** in the auto-import list above — it is resolved implicitly at runtime inside `CodedWorkflow` subclasses via the injected `system` accessor, but must be added explicitly when declaring an `ISystemService` parameter or variable in a plain `.cs` file.
>
> Some parameter and return types — and common operators over the values these methods return — live outside the auto-imported set. Add these as needed:
> - `using System.Linq;` — for LINQ operators such as `.Count()` / `.Where()` over the `IEnumerable<T>` returned by `ExtractTextOccurrences`, `ExtractEmails`, `ExtractUrls`, etc.
> - `using System.Security;` — for `SecureString` (credential methods)
> - `using System.Text.RegularExpressions;` — for `RegexOptions` (the `Replace` overload that takes match options)
> - `using UiPath.Core.Activities;` — for `PathType` (used by `GetResourceForLocalPath`) and `LogLevel`
> - `using UiPath.Platform.ResourceHandling;` — for `IResource`, `ILocalResource` (file operation return types)
...
### `Task<Dictionary<string, object>> InvokeProcessAsync(string processName, string folderPath = null, Dictionary<string, object> inArguments = null, Dictionary<string, object> inOutArguments = null, LogEntryType logEntry = LogEntryType.No, LogExitType logExit = LogExitType.No, UiPath.Core.Activities.LogLevel level = LogLevel.Info, InvokeProcessTargetSession targetSession = InvokeProcessTargetSession.Current, TimeSpan timeout = default, bool elevated = false)`
...
- `elevated` (`bool`) — When `true`, the (always isolated) invoked process runs at MediumPlus integrity. Requires the robot to enable the `UIPATH_MEDIUMPLUS_INVOKE` environment variable; otherwise the setting is silently ignored. Available for all target sessions (default: `false`)
...
### Use the system service in a plain C# class (non-CodedWorkflow)

Outside `CodedWorkflow`-derived classes the auto-imported namespaces are not available and the `system` accessor does not exist. Inject `ISystemService` as a constructor parameter and add the `using` directives explicitly.

```csharp
// Required in any plain .cs file — none of these are auto-imported outside CodedWorkflow
using UiPath.Core;                              // core UiPath types
using UiPath.Core.Activities.API;              // ISystemService
using UiPath.Core.Activities.Storage;          // IStorageFile, IStorageBucketFile
using UiPath.Orchestrator.Client.Models;       // OrchestratorJob, QueueItem, etc.
using UiPath.Activities.System.Jobs.Coded;     // RunJobOptionalParameters, LogEntryType, etc.

public class OrchestrationHelper
{
    private readonly ISystemService _system;

    public OrchestrationHelper(ISystemService system) => _system = system;

    public string TriggerJob(string processName, string folder, object args)
    {
        var (jobData, outputJson) = _system.RunJob(processName, folder, args);
        return outputJson;
    }

    public void ProcessNextQueueItem(string queueName, string folder)
    {
        var item = _system.GetTransactionItem(queueName, folder);
        if (item == null) return;
        // ... processing logic ...
        _system.SetTransactionStatus(item, ProcessingStatus.Successful);
    }
}
```

### Working with credentials and text processing (extra namespaces)

Credential methods return `SecureString`, and `Replace` accepts a `RegexOptions` value. These types live outside the auto-imported set and need explicit `using` directives even inside `CodedWorkflow` subclasses.

```csharp
// Extra usings needed in addition to the auto-imported set:
using System.Linq;                             // .Count() over IEnumerable<string> results
using System.Security;                         // SecureString
using System.Text.RegularExpressions;          // RegexOptions
using UiPath.Core.Activities;                  // PathType

[Workflow]
public void Execute()
{
    // GetCredential returns (string username, SecureString password)
    var (username, password) = system.GetCredential("ServiceAccount", "SharedAssets");
    Log($"Authenticating as: {username}");
    // Convert SecureString to plain text only where strictly required:
    // string plainPassword = new System.Net.NetworkCredential(string.Empty, password).Password;

    // ReadTextFile takes an IResource, not a path — wrap the path with GetResourceForLocalPath first
    var logResource = system.GetResourceForLocalPath(@"C:\Logs\output.txt", PathType.File);
    string rawLog = system.ReadTextFile(logResource);

    // Extract every line fragment that appears between "ERROR: " and the next line break
    var errors = system.ExtractTextOccurrences(rawLog, "ERROR: ", "\n", ignoreDuplicates: true);

    // Replace is the regex-capable method — its RegexOptions parameter is why System.Text.RegularExpressions is needed
    string masked = system.Replace(rawLog, @"password=\S+", "password=***", RegexOptions.IgnoreCase);

    Log($"Found {errors.Count()} error entries.");
}
```
