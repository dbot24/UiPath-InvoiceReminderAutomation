...
| Value | Description |
|-------|-------------|
| `Stop` | Sends a stop signal to the robot. The robot will finish its current activity and then stop gracefully. |
...
- `Stop` requests a graceful stop: the target robot will complete its currently executing activity before stopping. The job ends with status **Stopped**.
