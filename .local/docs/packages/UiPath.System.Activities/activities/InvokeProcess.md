...
Invokes another **UiPath process** (an Orchestrator process or a published package) within the current robot execution, waits for it to finish, and collects its output arguments.

> **Important.** This activity does **not** launch arbitrary Windows executables. Despite the generic name, the only thing it can start is another UiPath workflow — by Orchestrator process name (`UsePackage = False`) or by package name + entry-point path (`UsePackage = True`). To launch an external `.exe` (e.g. `7z.exe`, `powershell.exe`, a custom CLI tool), use **Invoke Code** with `System.Diagnostics.Process.Start`, **Invoke PowerShell**, or **Invoke VBScript** — there is no built-in UiPath activity that wraps a raw EXE launch in this package.
...
| `Elevated` | Elevated | `bool` | `False` | When `True`, the (always isolated) child executor runs at MediumPlus integrity. Requires the robot to set the `UIPATH_MEDIUMPLUS_INVOKE` machine environment variable; otherwise the flag is silently ignored. Windows only; available for all target sessions. |
...
- `Elevated` opts the isolated child executor into MediumPlus / UIAccess integrity without elevating the parent executor. Because **Invoke Process** always runs the invoked process in a separate (isolated) executor, the toggle is always available on Windows for every target session. It depends on the robot exposing `RuntimeFeatures.ElevatedInvoke`, advertised only when the machine env var `UIPATH_MEDIUMPLUS_INVOKE` is set; on robots that do not opt in, the flag is silently dropped. The legacy `UIPATH_MEDIUMPLUS_EXECUTOR` env var (which elevates the whole executor tree and breaks COM-based activities) is independent and unchanged.
