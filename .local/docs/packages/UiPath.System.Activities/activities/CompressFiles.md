...
|------|-------------|------|---------|-------------|
| `CompressionLevel` | Compression level | `ArchiveCompressionLevel` | `Normal` | Controls the trade-off between compression ratio and speed: `None`, `Fast`, `Normal`, or `Maximum`. |
| `EncryptionAlgorithm` | Encryption algorithm | `ArchiveEncryptionAlgorithm` | `Classic` | Algorithm used for password encryption when a password is specified: `Classic`, `AES128`, or `AES256`. |
